# -*- coding: utf-8 -*-
"""
Created on Mon Feb 28 11:37:22 2022
...
from idaes.gas_distribution.flowsheets.four_nodes_linear_network import (
    make_simple_model,
...
def get_nominal_demand_sequence():
    ...
    # 30 kg/hr -> kmol/hr
    val0 = pyo.value(50.0 * 1e4 / 18.0)
    # 50 SCM/hr -> kmol/hr
    val1 = pyo.value(70.0 * 1e4 * 0.72 / 18.0)
...
def solve_boost_pressure_given_demand(demand):
    ...
    m.fs.nodes[0].state[t0].pressure.fix(70.0*pyo.units.bar)
    ...
    var_near_bound = variables_near_bounds_generator(m, tol = 10**-5)
    return m
